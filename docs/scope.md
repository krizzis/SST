...
- Structured scene-state extraction for attributes such as outfit, pose, emotion, location, and similar present-state descriptors.
...
- Autonomous generation of brand-new scene facts that are not grounded in the current turn pair.
...
| SillyTavern host environment | Platform dependency | Provides extension APIs, chat context, background controls, and image pipeline hooks |
...
- **Prompt determinism may drift if extraction relies on free-form text** -> Mitigation: define canonical structured fields and deterministic serialization order.
